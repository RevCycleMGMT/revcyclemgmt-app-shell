...
import { PatientCard } from "@/components/pipeline/PatientCard";
...
export default function EncounterPage({ params }: EncounterPageProps) {
  ...
  const teachingNote =
    scenario.teachingNotes.find((note) => note.step === "encounter")?.text ??
    "Review the clinical documentation before assigning codes.";
      ...
      <div className="mx-auto w-full max-w-7xl space-y-8">
        ...
        <section>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
            Read the source documentation before revealing diagnosis, procedure,
            modifier, and claim-building logic.
          </p>
        </section>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 p-4">
          <Button asChild variant="ghost" className="rounded-lg">
            <Link href="/demo">&larr; Back to Scenarios</Link>
          </Button>
          <Button asChild className="rounded-lg bg-teal-600 text-white hover:bg-teal-700">
            <Link href={`/demo/${scenario.id}/coding`}>
              Continue to Coding &rarr;
            </Link>
          </Button>
        </div>
      </nav>
